...
def check_and_remind_inactive_presales():
    """
    Mengecek presales yang tidak memiliki aktivitas > 30 hari atau tidak pernah beraktivitas sama sekali.
    Lalu mengirimkan email rekap ke krisa.kurniawan@sisindokom.com.
    """
    try:
        with engine.connect() as conn:
            # Query untuk mencari presales yang last_activity-nya lebih dari 30 hari yang lalu
            # atau yang belum pernah punya log activity sama sekali (IS NULL)
            query = text("""
                SELECT 
                    p.presales_name, 
                    p.email,
            ...
            # Jika tidak ada yang melanggar SLA, hentikan fungsi (tidak perlu kirim email)
            if not inactive_users:
                return {"status": 200, "message": "Semua Presales aktif dalam 1 bulan terakhir. Aman! ✅"}

            # --- RANGKAI BODY EMAIL UNTUK KRISA ---
            list_html = ""
            for user in inactive_users:
                # Format tanggal agar mudah dibaca
                if pd.notnull(user['last_activity']):
            ...
            email_body = f"""
            <h3>🚨 Presales Inactivity Alert (> 1 Bulan)</h3>
            <p>Halo Krisa,</p>
            <p>Sistem mendeteksi ada beberapa akun Presales yang <b>tidak melakukan aktivitas (input/update opportunity) selama lebih dari 1 bulan</b> di Presales App.</p>
            <p>Berikut adalah daftarnya:</p>
            <ul>
                {list_html}
            </ul>
            <p><b>Action Required:</b></p>
            <p>Mohon untuk melakukan <i>follow-up</i> kepada nama-nama di atas. Jika memang belum ada <i>opportunity</i> baru untuk diinput, mereka diwajibkan untuk me-<i>reply</i> email *reminder* dari Anda sebagai bentuk konfirmasi.</p>
            <br>
            <p style='font-size: 10px; color: grey;'>System Auto-Generated by Presales App</p>
            """

            # Kirim email ke Krisa
            target_email = "krisa.kurniawan@sisindokom.com"
